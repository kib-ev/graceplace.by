    ...
    <div class="container">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <h5 class="card-title">Бронь ячейки №{{ $appointment->id }}</h5>
...
{{--                        </select>--}}
        ...
        <form action="{{ route('admin.appointments.payments.store') }}" method="POST" autocomplete="off">
            ...
            <div class="mb-3">
                <label for="amount" class="form-label">Сумма</label>
                <input type="datetime-local" name="amount" id="amount" class="form-control" required value="{{ $appointment->end_at->format('Y-m-d H:i') }}">
            </div>
